...
public class KeybindComponent extends SettingComponent {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (listening) {
            if(font.isEnabled()) {
                Notorious.INSTANCE.fontRenderer.drawStringWithShadow("Bind: Listening...", x + 9f, y + 2f, Color.WHITE);
            }else {
                Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("Bind: Listening...", x + 9f, y + 2f, new Color(255, 255, 255).getRGB());
            }
        } else {
            if(font.isEnabled()) {
                Notorious.INSTANCE.fontRenderer.drawStringWithShadow("Bind: <" + Keyboard.getKeyName(setting.getBind()) + ">", x + 9f, y + 2f, Color.WHITE);
            }else {
                Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("Bind: <" + Keyboard.getKeyName(setting.getBind()) + ">", x + 9f, y + 2f, new Color(255, 255, 255).getRGB());
            }
        }
    }
