...
public class BooleanComponent extends SettingComponent {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        Gui.drawRect(x, y, x + width, y + height, setting.isEnabled() ? color : new Color(0, 0, 0, 150).getRGB());
        Gui.drawRect(x, y, x + 2, y + height, color);
        if(font.isEnabled()) {
            Notorious.INSTANCE.fontRenderer.drawStringWithShadow(setting.getName(), x + 9f, y + 2f, Color.WHITE);
        }else {
            Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(setting.getName(), x + 9f, y + 2f, new Color(255, 255, 255).getRGB());
        }
    }
